...
def extract_taxa(
    ...
    for i, rank in enumerate(ranks):
        try:
            item = (
                items[i][3:].replace("unclassified ", "unclassified.").replace(" ", "_")
            )
        except IndexError:
            item = ""
        data[rank] = item
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--taxcol", default="Taxon", help="Taxonomy column (default: Taxon)"
    )
    parser.add_argument(
        "-r",
        "--ranks",
        ...
        help="Taxonomic ranks (default: kingdom phylum class order family genus species)",
    )
