...
def main(args):
    ...
    agrees = update.loc[
        (update[args.agree_rank] == update["base"])
        & (update[(args.agree_rank)].isin(args.agree_taxa))
    ]
    ...
    for rank in args.update_ranks:
        ...
        classified_in_update = agrees.loc[
            ~agrees[rank].str.startswith("unclassified")
        ].index.tolist()
    ...
    if len(args.downstream_ranks) > 0:
        ...
        ranks = [args.update_ranks[-1]] + args.downstream_ranks
        sys.stderr.write(
            f"Updating downstream ranks {' '.join(args.downstream_ranks)}\n"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-b", "--base", help="Base taxonomy file (to be updated)", required=True
    )
    parser.add_argument(
        "-u", "--update", help="Updated taxonomy file(s)", nargs="+", required=True
    )
    parser.add_argument(
        "-a",
        "--agree_rank",
        help="Only update entries that agree on <agree_rank>",
        default="Class",
    )
    parser.add_argument(
        "-t",
        "--agree_taxa",
        help="Taxa that must agree on <agree_rank>",
        nargs="+",
        default=["Insecta"],
    )
    parser.add_argument(
        "-U", "--update_ranks", help="Ranks to update", nargs="+", default=["Order"]
    )
    parser.add_argument(
        "-d",
        "--downstream_ranks",
        help="Ranks downstream of the update ranks for which to update 'unclassified' string",
        nargs="+",
    )
    parser.add_argument(
        "-r", "--rename_file", help="File used to map ids from update to base"
    )
    parser.add_argument(
        "-c", "--rename_col", help="Column in rename_file to use as id for mapping"
    )
