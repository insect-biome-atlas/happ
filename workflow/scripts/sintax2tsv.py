...
def parse_taxonomy(
    ...
    tax_values = [
        x[2:].replace("unclassified ", "unclassified.")
        for x in r.sub("", tax).split(",")
    ]
    tax_values = [x.replace(" ", "_") for x in tax_values]
    d = {asv_id: {}}
    last_known = ""
...
def main(infile, outfile, conf_outfile, ranks, cutoff):
    ...
    parsed = (
        sintax.groupby(level=0)
        .apply(parse_taxonomy, r=r, ranks=ranks, cutoff=cutoff)
        .reset_index()
        .drop("level_1", axis=1)
        .set_index(0)
    )
    ...
    parsed, conf = (
        parsed.loc[:, [x for x in parsed.columns if not x.endswith("_conf")]],
        parsed.loc[:, [x for x in parsed.columns if x.endswith("_conf")]],
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--cutoff", type=float, default=0.8, help="Sintax cutoff (default=0.8)"
    )
    ...
    parser.add_argument(
        ...
        help="Ranks used in Sintax assignment",
    )
    args = parser.parse_args()
    main(
        infile=args.infile,
        outfile=args.outfile,
        conf_outfile=args.conf_out,
        ranks=args.ranks,
        cutoff=args.cutoff,
    )
